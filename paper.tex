...
of the computations on the nodes of the database cluster. 
We extend the scientific database cluster with the introduction of an application-aware cache for query results, building on the concept of semantic caching.
...
threshold and top-$k$ queries usually have to examine the entire data volume of a time-step or a signification portion of it. Furthermore, the data product 
of threshold and top-$k$ 
...
reuse for the results of these queries is extremely small. This is why the cache currently stores only the results of threshold queries and is therefore a 
limited version of a semantic cache. Nevertheless, it can easily be extended to cache the results of other query types as well if that becomes advantageous.
...
Server) as opposed to designing a new database system from the ground up as is the case in \cite{Andrade}.
