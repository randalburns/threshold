...
interested in out of the box. In this paper we present a method for the efficient evaluation of threshold queries of derived fields for large numerical 
simulation datasets
...
It is impractical to materialize all possible derived fields due to the large size of the datasets and the limits of available storage. Therefore, the derived fields 
and threshold queries on them have to be evaluated on-demand from the stored data.
...
we cache the actual results of the computation, which leads to query performance improvement of up to X times.
...
the locations with norms of the vorticity above thresholds at different levels. For the first set the threshold was set high and only $\sim$4,300 points 
(or 0.0004\% of all points) were above the threshold. For the second set a medium threshold was chosen and $\sim$87,000 points 
(or 0.0081\% of all points) were above the threshold. Finally, a low threshold was chosen for the last set with $\sim$900,000 points 
...
The execution times were averaged over 10 runs and cache entries for the particular time-step were dropped before each run. 
We utilized 4 threads per database
node for the evaluation of each query. As we can see from the results the overhead associated with querying the cache first is small, less than 3\% and within
the margin of error. Additionally, the method shows stable running time across different time-steps and threshold levels
...
queries and threshold values as for the runs shown in Fig. \ref{fig:cache_overhead} but with varying number of threads per node. The computations for all of 
...
observed for the I/O when additional threads are utilized. SQL Server already makes use of parallelism internally and the data have to be retrieve from the
