...
\begin{figure}
\centering
\includegraphics[width=3.35in]{Figures/vorticity_histogram.pdf}
\caption{Tail of the histogram of the norm $\omega$ of the vorticity field over the isotropic Navier-Stokes dataset. $\sqrt{\langle \omega ^2\rangle}$ is computed over the entire dataset.}
\label{fig:pdf}
\end{figure}
...
and make subsequent analysis queries as needed in order to study these events. The relationship between different worms that connect and reconnect
...
Fig. \ref{fig:pdf} shows the distribution of the large values of the norm of the vorticity for the isotropic Navier-Stokes dataset.
This result can be used to estimate the number of points returned by a given threshold value (the integral of this curve must simply be divided by the total number of frames).

\begin{figure}[h]
\centering
\includegraphics[width=3.35in]{Figures/most_intense_cluster.png}
\caption{3D (fixed frame) cut through the 4D cluster containing the most intense event.
Lines parallel to the axes are given for reference: red, green and blue correspond to $x$, $y$ and $z$, respectively;
their lengths are the extents of the structure on each axis, which are approximately $0.55 \pi \times 0.40 \pi \times 0.17 \pi$.}
\label{fig:max_vort_cluster}
\end{figure}

Fig. \ref{fig:max_vort_cluster} shows the most intense event observed in the MHD dataset. The locations of maximum vorticity in the dataset were clustered
...
The timespan stored is equal to about one large eddy turnover time. It is not clear whether a different more intense event could emerge from an existing
cluster if the timespan stored was larger. Fig. \ref{fig:max_vort_cluster} also shows that most interactions between worms are not simple. 
...
\textcolor{red}{Chichi: The following refers to the PDF made by Kalin; should we include that one as well or change this text?
I just realized that the values are different since Kalin used the MHD dataset...
Not only that, the statistics will be different as well, since the structures have different shapes.}
...
an array database system build from the ground up. It partitions arrays vertically into overlapping chunks. SciDB provides a declarative Array Query Language (AQL)
and an Array Functional Language (AFL). Users can create arrays with named dimensions with AQL and make use of the functional operators defined in AFL, such
...
within the MoneDB framework \cite{MonetDB}.
